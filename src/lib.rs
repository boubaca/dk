//! # Overview
//! This code defines functionality for a hardware abstraction layer for an embedded device,
//! including peripheral management (e.g., LEDs, buttons, timer, etc.), USB devices, and radio communication.
//! It provides a `Board` structure that consolidates device components and peripheral interfaces for user-friendly operations.
//!
//! # Features
//! - **LED and Button Control:** APIs to interact with LEDs and Buttons to check their state or control their status.
//! - **Timers:** Blocking delays that can be used for task timing.
//! - **Peripheral Integration:** Supports USB and radio modules when specific compile-time features (`advanced`, `radio`, `usbd`) are enabled.
//! - **Runtime Logging:** Comprehensive logging provided by `defmt` for debugging and tracing device behavior.
//!
//! ## Conditional Compilation
//! The code contains modules and functionality specific to certain use cases, controlled by feature flags:
...
/// Represents a Button with an embedded pin configured as an input with push-pull drive.
///
/// # Fields
/// - `inner`: The wrapped pin, configured as an input with push-pull drive mode.
///
/// This structure is typically used to represent and control a physical button in an embedded system,
/// where the pin is configured to read the button's state.
///
/// # Generics
/// `Input<PushPull>`: A hardware abstraction layer (HAL) type representing the pin configured as
/// an input with a push-pull setting, ensuring proper handling of digital signals.
///
/// # Example
/// ```rust
///  // Update with the actual path/module where Button is defined
///
pub struct  Button {
    inner: Pin<Input<PullUp>>,
}

/// Struct representing a collection of buttons.
///
/// The `Buttons` struct provides a container for four distinct `Button`
/// objects. Each button is represented as a public field within the structure.
///
/// # Fields
///
/// * `_1` - Represents the first button.
/// * `_2` - Represents the second button.
/// * `_3` - Represents the third button.
/// * `_4` - Represents the fourth button.
pub struct Buttons {
    /// A public field of a struct representing a Button component.
    ///
    /// This field `_1` is an instance of the `Button` type, allowing interaction
    /// with or manipulation of a button component in the user interface.
    ///
    /// Note: This field name (`_1`) is a placeholder and may not follow typical
    /// naming conventions. Consider renaming the field to something more descriptive
...
impl Button {
    /// is_pressed returned true if button pressed
    pub fn is_pressed(&mut self) -> bool {
        self.inner.is_low().unwrap_or_else(|_| false)
    }
